 ...
 *
 * ВАЖНО: finalIndex считается ОТДЕЛЬНО для каждого канала (говорящего) —
 * то есть channelTag="0", finalIndex="0" и channelTag="1", finalIndex="0"
 * — это две РАЗНЫЕ фразы, не дубликат. Поэтому ключ дедупликации должен
 * быть составным: `${channelTag}:${finalIndex}`, а не просто finalIndex.
 *
 * Порядок фраз восстанавливается по startTimeMs (реальному времени начала
 * фразы в аудиозаписи), а не по finalIndex — так фразы разных говорящих
 * естественно перемешиваются в хронологическом порядке диалога.
 */
function parseSpeakerLabeledResult(rawText) {
  ...
  for (const line of lines) {
    ...
    const channelTag = result.channelTag || "0";
    const key = `${channelTag}:${finalIndex}`;
    ...
    if (result.finalRefinement?.normalizedText?.alternatives?.length) {
      ...
      const startTimeMs = Number(alt.startTimeMs || 0);
      byKey.set(key, { channelTag, text: alt.text || "", startTimeMs });
    } else if (result.final?.alternatives?.length && !byKey.has(key)) {
  ...
  // Сортируем по реальному времени начала фразы в записи — это даёт
  // правильный хронологический порядок диалога между говорящими
  const utterances = [...byKey.values()]
