...
export async function checkRecognitionStatus(operationId) {
  ...
  // ВРЕМЕННОЕ логирование для диагностики дублирования текста в результатах —
  // удалить после того, как разберёмся со структурой ответа.
  console.log("SpeechKit raw result (первые 3000 символов):", resultText.slice(0, 3000));
 ...
 * по одному JSON-объекту на строку) и собирает читаемый транскрипт
 * с метками говорящих, объединяя соседние фразы одного говорящего.
 *
 * Возвращает массив фраз вида { speakerTag, text } — преобразование
 * в текст с подписями "Терапевт:"/"Клиент:" происходит на уровне
 * API-эндпоинта (recognize.js), а не здесь, чтобы можно было легко
 * поменять, кто из говорящих terapevт, а кто клиент.
 */
function parseSpeakerLabeledResult(rawText) {
  ...
  for (const line of lines) {
    ...
    // Берём ТОЛЬКО финальный результат (final), пропускаем промежуточные
    // (partial) и не дублируем normalized/raw варианты одной фразы —
    // используем единственный объект finalRefinement, если он есть,
    // иначе обычный final, но не оба одновременно.
    const finalResult = parsed.result?.finalRefinement?.normalizedText
      ? parsed.result.finalRefinement.normalizedText
      : parsed.result?.final;

    if (!finalResult || !finalResult.alternatives || finalResult.alternatives.length === 0) {
      continue;
    }

    const bestAlternative = finalResult.alternatives[0];
    if (!bestAlternative || !bestAlternative.text) continue;

    const speakerTag = bestAlternative.words?.[0]?.speakerTag || finalResult.channelTag || "1";

    utterances.push({ speakerTag, text: bestAlternative.text });
  }
  ...
  for (const utterance of utterances) {
    ...
    if (last && last.speakerTag === utterance.speakerTag) {
      last.text += " " + utterance.text;
    } else {
      grouped.push({ speakerTag: utterance.speakerTag, text: utterance.text });
    }
  }
