...
// PUT /api/settings -> обновить формат заметки по умолчанию
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const result = await sql`
      SELECT email, name, default_note_format, custom_format_instructions
  ...
  if (req.method === "PUT") {
    const { defaultNoteFormat, customFormatInstructions, name } = req.body || {};
    try {
      const result = await sql`
        UPDATE therapists
        ...
        RETURNING email, name, default_note_format, custom_format_instructions
      `;
      return res.status(200).json({ settings: result[0] });
    } catch (err) {
